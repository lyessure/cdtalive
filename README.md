...
- **开箱即用的控制台**：查看流量、余额、实例状态与停机记录，直接调整分组开机时段和开关机模式。
- **配置持久化**：配置和运行数据保存在本地 `data/` 目录，容器重建后无需重新设置。

服务提供一个中文控制台，可查看余额、当月 CDT 流量、ECS 状态、运行时长和近 30 天停机记录，并可按星期分组配置每日开机时段与开关机模式。
...
- 在流量和余额均处于安全范围、且处于开机时段内时，自动拉起处于停止状态的目标实例；离开开机时段后自动停机。
- 支持按星期分组设置一个或多个定时开机时间段，包含跨天时间段，例如工作日和周末可以使用不同时间。
...
4. 账户余额低于阈值：停止账号下的全部 ECS 实例。
5. 处于配置的开机时段：确认目标实例运行；若实例已被抢占回收或处于停止状态，则发起启动请求。
6. 开机时段之外：停止配置的目标 ECS 实例。
...
| `CDT_DAILY_START_SCHEDULES` | 开机规则列表，使用 JSON/YAML 字符串表示 | 空 |
| `CDT_POWER_MODE` | `on`、`auto` 或 `off` | `auto` |
| `CDT_WEB_PORT` | 宿主机 Web 端口，仅 Compose 使用 | `5201` |
| `CDT_RUNTIME` | Docker 构建版本，`python` 或 `go` | `python` |

`daily_start_schedules` 的每条规则包含 `weekdays` 和 `windows`。星期使用 1-7 表示周一至周日；时间段格式为 `HH:MM-HH:MM`，例如 `22:00-07:00` 表示跨天开机。结束时间允许使用 `24:00`。跨天窗口按开始日期归属，例如选中周一的 `22:00-07:00` 会覆盖周一晚间至周二早晨。
旧版本的 `daily_stop_windows` 与 `daily_stop_weekdays` 配置会在读取时自动转换为开机时段；保存新设置后将使用新的 `daily_start_schedules` 字段。
...
服务在 ECS 启停过渡状态期间会每 15 秒仅刷新实例状态；其他情况下按配置的检查间隔执行完整检查。到达每日开机时段的开始或结束时间时，会额外立即执行一次完整检查，且不会改变原有的常规检查周期。
...
| `GET` | `/api/settings` | 当前分组开机时段与电源模式 |
| `PUT` | `/api/settings` | 更新分组开机时段与电源模式，并触发后台检查 |
